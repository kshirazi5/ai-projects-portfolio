...
# Optional parsers (guard-import so app still loads without them)
try:
    import fitz  # PyMuPDF
except Exception:
    fitz = None
...
def resume_matcher_app():
    ...
    if run:
        ...
        try:
            _, present, missing = keyword_tables(jd_text, resume_text, top_k=30)
            c1, c2 = st.columns(2)
            with c1:
